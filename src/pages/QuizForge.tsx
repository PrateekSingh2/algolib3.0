...
export default function QuizForge() {
  ...
  if (isAuthLoading) return <div className="min-h-screen bg-[#050505] flex items-center justify-center"><Loader2 className="animate-spin text-indigo-500" size={40} /></div>;
    ...
    <div className="min-h-screen bg-[#050505] text-zinc-200 font-sans relative overflow-x-hidden">
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md p-6">
             <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="max-w-lg w-full bg-[#111] border border-white/[0.08] p-10 rounded-[2.5rem] shadow-2xl text-center">
                ...
                <div className="bg-black/60 border border-white/[0.05] rounded-2xl p-4 flex items-center justify-between gap-4 mb-8 shadow-inner">
                    <div className="flex items-center gap-3 overflow-hidden text-zinc-300"><LinkIcon size={18} className="text-zinc-500 shrink-0" /><span className="text-sm font-mono truncate select-all">{generatedLink}</span></div>
                    <button onClick={copyLinkToClipboard} className="p-2.5 bg-white/[0.05] hover:bg-white/[0.1] rounded-xl transition-colors shrink-0 text-white">{isCopied ? <Check size={18} className="text-emerald-400" /> : <Copy size={18} />}</button>
                </div>
          ...
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[300] flex items-center justify-center bg-black/80 backdrop-blur-md p-4 md:p-6">
             <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} exit={{ scale: 0.95, y: 20, opacity: 0 }} className="max-w-5xl w-full max-h-[90vh] bg-[#0a0a0a] border border-white/[0.08] rounded-[2.5rem] shadow-2xl flex flex-col overflow-hidden relative">
                <div className="p-6 md:p-8 border-b border-white/[0.05] flex justify-between items-center bg-black/40 sticky top-0 z-10">
                   <div><h2 className="text-2xl md:text-3xl font-bold text-white flex items-center gap-3 tracking-tight"><Activity className="text-indigo-500" size={28} /> {activeQuizTitle}</h2></div>
                   <button onClick={() => setShowResultsModal(false)} className="p-3 bg-white/5 hover:bg-rose-500/20 text-zinc-400 hover:text-rose-400 rounded-xl transition-colors"><X size={24} /></button>
                </div>
                   ...
                   {isFetchingResults ? (
                       ...
                       <>
                           <div className="grid grid-cols-1 sm:grid-cols-3 gap-5 mb-10">
                               <div className="bg-white/[0.02] border border-white/[0.05] p-6 rounded-3xl flex flex-col shadow-inner"><div className="flex items-center gap-3 text-zinc-500 mb-2"><Users size={18} /><span className="text-[11px] font-bold uppercase tracking-wider">Total Attempts</span></div><span className="text-4xl font-mono font-bold text-white tracking-tight">{activeResults.length}</span></div>
                               <div className="bg-white/[0.02] border border-white/[0.05] p-6 rounded-3xl flex flex-col shadow-inner"><div className="flex items-center gap-3 text-indigo-400 mb-2"><Target size={18} /><span className="text-[11px] font-bold uppercase tracking-wider text-indigo-400/80">Avg Score</span></div><span className="text-4xl font-mono font-bold text-white tracking-tight">{averageScore} <span className="text-lg text-zinc-600 font-normal">/ {totalQuestions}</span></span></div>
                               <div className="bg-white/[0.02] border border-white/[0.05] p-6 rounded-3xl flex flex-col shadow-inner"><div className="flex items-center gap-3 text-amber-500 mb-2"><Trophy size={18} /><span className="text-[11px] font-bold uppercase tracking-wider text-amber-500/80">Highest Score</span></div><span className="text-4xl font-mono font-bold text-white tracking-tight">{activeResults.length > 0 ? activeResults[0].score : 0} <span className="text-lg text-zinc-600 font-normal">/ {totalQuestions}</span></span></div>
                           </div>
                           <div className="border border-white/[0.05] rounded-3xl overflow-hidden bg-black/40">
                               <div className="overflow-x-auto">
                                   <table className="w-full text-left border-collapse">
                                       <thead><tr className="border-b border-white/[0.05] bg-white/[0.02]"><th className="py-4 px-6 font-bold text-zinc-500 text-[11px] uppercase tracking-widest w-20 text-center">Rank</th><th className="py-4 px-6 font-bold text-zinc-500 text-[11px] uppercase tracking-widest">Candidate</th><th className="py-4 px-6 font-bold text-zinc-500 text-[11px] uppercase tracking-widest">Score</th><th className="py-4 px-6 font-bold text-zinc-500 text-[11px] uppercase tracking-widest hidden sm:table-cell">Duration</th><th className="py-4 px-6 font-bold text-zinc-500 text-[11px] uppercase tracking-widest text-center">Violations</th></tr></thead>
                                       <tbody>
      ...
      <main className="pt-32 pb-20 px-4 md:px-6 container mx-auto max-w-4xl relative z-10">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
            <div>
                <h1 className="text-3xl font-bold text-white tracking-tight flex items-center gap-3"><ShieldAlert size={28} className="text-indigo-500" /> My Dashboard</h1>
        ...
        {quizView === "manager" ? (
            <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="bg-[#0a0a0a]/60 border border-white/[0.06] p-8 md:p-10 rounded-[2.5rem] shadow-2xl">
                ...
                {existingQuizzes.length === 0 ? (
                    <div className="py-16 text-center border border-dashed border-white/10 rounded-3xl"><HelpCircle size={48} className="text-zinc-600 mx-auto mb-4" /><p className="text-zinc-400 font-medium text-lg mb-1">No assessments found.</p><p className="text-zinc-500 text-sm">Create your first quiz to see it here.</p></div>
                ) : (
                        ...
                        {existingQuizzes.map(q => (
                            <div key={q.id} className="p-6 bg-black/40 border border-white/[0.06] rounded-[1.5rem] hover:border-indigo-500/30 hover:bg-indigo-500/5 transition-all group">
                                ...
                                <div className="space-y-2">
                                    <div className="flex items-center gap-2 text-xs text-zinc-500 font-mono"><Hash size={14}/> Code: <span className="text-zinc-300 font-bold">{q.join_code || q.id.substring(0, 8)}</span></div>
                                    <div className="flex items-center gap-2 text-xs text-zinc-400"><Calendar size={14}/> {new Date(q.start_time).toLocaleString()}</div>
                                    <div className="flex items-center gap-2 text-xs text-zinc-400"><Clock size={14}/> Dedicated Time: <span className="text-indigo-400 font-bold">{q.duration_seconds ? q.duration_seconds / 60 : 45} mins</span></div>
                                </div>
                            </div>
                        ))}
                    </div>
                ...
                <div className="bg-gradient-to-br from-[#110505] to-[#0a0a0a] border border-rose-500/20 p-6 md:p-8 rounded-[2rem] shadow-[inset_0_0_40px_rgba(244,63,94,0.03)]">
                  ...
                  <div className="mb-6">
                      ...
                      <div className="flex flex-col md:flex-row gap-3 mt-3">
                          <button onClick={() => setQuizForm({...quizForm, maxWarnings: 5})} className={`flex-1 p-4 rounded-xl border-2 transition-all text-left ${quizForm.maxWarnings >= 5 ? 'bg-emerald-500/10 border-emerald-500/50' : 'bg-black/40 border-white/5 hover:border-white/20'}`}><h4 className={`font-bold mb-1 ${quizForm.maxWarnings >= 5 ? 'text-emerald-400' : 'text-zinc-300'}`}>Lenient</h4><p className="text-[11px] text-zinc-500">Allows up to 5 tab switches. Good for open-book.</p></button>
                          <button onClick={() => setQuizForm({...quizForm, maxWarnings: 3})} className={`flex-1 p-4 rounded-xl border-2 transition-all text-left relative overflow-hidden ${quizForm.maxWarnings === 3 ? 'bg-amber-500/10 border-amber-500/50' : 'bg-black/40 border-white/5 hover:border-white/20'}`}><span className="absolute top-3 right-3 text-[9px] font-bold uppercase tracking-wider bg-amber-500/20 text-amber-400 px-2 py-0.5 rounded-md">Recommended</span><h4 className={`font-bold mb-1 pr-16 ${quizForm.maxWarnings === 3 ? 'text-amber-400' : 'text-zinc-300'}`}>Standard</h4><p className="text-[11px] text-zinc-500">Flags after 3 violations. Standard testing mode.</p></button>
                          <button onClick={() => setQuizForm({...quizForm, maxWarnings: 1})} className={`flex-1 p-4 rounded-xl border-2 transition-all text-left ${quizForm.maxWarnings <= 1 ? 'bg-rose-500/10 border-rose-500/50' : 'bg-black/40 border-white/5 hover:border-white/20'}`}><h4 className={`font-bold mb-1 flex items-center gap-2 ${quizForm.maxWarnings <= 1 ? 'text-rose-400' : 'text-zinc-300'}`}>Ultra Strict <Activity size={14}/></h4><p className="text-[11px] text-zinc-500">1-strike policy. Enforces full-screen tracking.</p></button>
                      </div>
                  </div>
                  <div className="bg-black/50 border border-white/5 rounded-xl p-4 flex gap-4 items-start"><Fingerprint className="text-rose-500 shrink-0 mt-1" size={20} /><div><p className="text-sm text-zinc-300 font-medium mb-1">Active Modules based on your selection:</p><ul className="text-xs text-zinc-500 space-y-1 list-disc pl-4 marker:text-rose-500"><li>Disables Copy, Paste, and Right-Click.</li><li>Tracks window blur (clicking outside the browser).</li><li>Auto-submits exam when the <b>{quizForm.maxWarnings}</b> violation limit is hit.</li></ul></div></div>
                </div>

                {/* Questions Builder */}
                <div className="bg-[#0a0a0a]/80 border border-white/[0.06] p-6 md:p-8 rounded-[2rem]">
                    <div className="flex justify-between items-center mb-6 border-b border-white/[0.05] pb-4"><h3 className="text-lg font-bold text-white flex items-center gap-2"><HelpCircle size={18} className="text-sky-400"/> Assessment Questions</h3><button onClick={addNewQuestion} className="px-4 py-2 bg-sky-500/10 hover:bg-sky-500/20 text-sky-400 rounded-xl text-sm font-bold flex items-center gap-2"><Plus size={16}/> Add Question</button></div>
                    <div className="space-y-6">
                        {quizForm.questions.map((q, qIndex) => (
                            <div key={qIndex} className="p-6 bg-black/40 border border-white/[0.08] rounded-3xl relative">
                                <div className="absolute top-5 right-5 flex gap-2 items-center"><span className="text-xs font-bold text-zinc-500 bg-white/5 px-2.5 py-1 rounded-md">Q{qIndex + 1}</span><button onClick={() => setQuizForm({...quizForm, questions: quizForm.questions.filter((_, i) => i !== qIndex)})} className="text-zinc-600 hover:text-rose-400 p-1.5 rounded-md transition-colors" title="Delete Question"><Trash2 size={16}/></button></div>
                                ...
                                <div className="mb-5 bg-white/[0.02] p-1.5 rounded-xl w-full md:w-fit flex flex-wrap gap-1 border border-white/5"><button onClick={() => changeQuestionType(qIndex, 'single')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${q.questionType === 'single' ? 'bg-indigo-500/20 text-indigo-400' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}`}><CircleDot size={14}/> Single Choice</button><button onClick={() => changeQuestionType(qIndex, 'multiple')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${q.questionType === 'multiple' ? 'bg-indigo-500/20 text-indigo-400' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}`}><CheckSquare size={14}/> Multiple Choice</button><button onClick={() => changeQuestionType(qIndex, 'true_false')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${q.questionType === 'true_false' ? 'bg-indigo-500/20 text-indigo-400' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}`}><ToggleLeft size={14}/> True / False</button><button onClick={() => changeQuestionType(qIndex, 'numerical')} className={`flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-xs font-bold transition-all ${q.questionType === 'numerical' ? 'bg-indigo-500/20 text-indigo-400' : 'text-zinc-500 hover:text-zinc-300 hover:bg-white/5'}`}><NumIcon size={14}/> Numerical</button></div>
                                                ...
                                                {q.options.map((opt, oIndex) => {
                                                    ...
                                                    return (
                                                        <div key={opt.id} className="flex items-center gap-3">
                                                            ...
                                                            <div className="relative flex-1"><div className="absolute left-3 top-1/2 -translate-y-1/2 text-zinc-600 font-bold text-sm select-none">{letter}.</div><textarea rows={1} value={opt.text} onChange={(e) => { const newOpts = [...q.options]; newOpts[oIndex].text = e.target.value; updateQuizQ(qIndex, 'options', newOpts); }} className={`${inputClass} !py-2.5 pl-10 text-[13px] font-mono min-h-[44px] resize-y`} placeholder="Enter option... (Use ``` code ``` for code)" /></div>
