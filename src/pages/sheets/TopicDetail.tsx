    ...
    <div className="min-h-screen bg-[#050505] text-zinc-300 font-sans flex flex-col">
      ...
      <main className="relative z-10 flex-1 w-full max-w-[1100px] mx-auto px-4 sm:px-6 pt-28 md:pt-36 pb-24">
        ...
        <motion.div
          ...
          className="mb-8"
        >
          <Link to="/sheets" className="inline-flex items-center gap-2 text-sm font-medium text-zinc-400 hover:text-white transition-colors">
            <ArrowLeft size={16} />
              ...
              return (
                <button
                  ...
                  className={`shrink-0 px-4 py-2 rounded-full text-[13px] font-semibold transition-all duration-200 border whitespace-nowrap ${
                    isActive
                      ? 'text-white border-white/20 bg-white/10 shadow-[0_0_12px_rgba(255,255,255,0.06)]'
                      : 'text-zinc-400 border-white/[0.06] bg-white/[0.02] hover:text-white hover:bg-white/[0.08] hover:border-white/[0.1]'
                  }`}
        ...
        <motion.section
          ...
          className="rounded-2xl border border-white/[0.07] bg-white/[0.015] backdrop-blur-sm overflow-hidden shadow-[0_8px_40px_rgba(0,0,0,0.4)]"
        >
          {/* Table header */}
          <div className="hidden sm:grid grid-cols-[2.5rem_1fr_auto] items-center px-6 py-3 bg-white/[0.015] border-b border-white/[0.05]">
            <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest">Sl no.</span>
            <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest">Topic</span>
            <span className="text-[10px] font-semibold text-zinc-600 uppercase tracking-widest pr-2 text-right">Action</span>
          </div>
          ...
          {loading ? (
            ...
            <div className="flex flex-col items-center justify-center py-24 text-center px-6">
              ...
              <h3 className="text-zinc-200 font-semibold text-lg mb-2">Coming Soon</h3>
              <p className="text-zinc-500 text-sm">Problems for <span className="text-white font-medium">{topic.title}</span> are being authored. Check back soon.</p>
            </div>
          ) : (
              ...
              {algorithms.map((problem, index) => {
                 return (
                  <Link
                    key={problem.id}
                    to={`/view/${problem.id}`}
                    className="group flex flex-col sm:grid sm:grid-cols-[2.5rem_1fr_auto] items-start sm:items-center px-4 sm:px-6 py-4 hover:bg-white/[0.025] transition-colors"
                  >
                    <span className="hidden sm:block shrink-0 w-8 text-[12px] text-zinc-600 font-mono">
                      {String(index + 1).padStart(2, '0')}
                    </span>
          
                    <div className="flex-1 min-w-0 pr-4 w-full">
                      <div className="flex items-center gap-2 mb-1">
                        <span className="text-[15px] font-semibold text-zinc-200 group-hover:text-white group-hover:text-sky-400 transition-colors leading-snug truncate">
                          {problem.title}
                        </span>
                      </div>
                      <p className="text-[13px] text-zinc-400 line-clamp-2 leading-relaxed mt-1">
                        {problem.description}
                      </p>
                    </div>
                    ...
                    <div className="shrink-0 flex items-center gap-2 mt-3 sm:mt-0 ml-auto">
                      <button
                        ...
                        className="p-2 rounded-lg text-zinc-500 hover:text-white hover:bg-white/[0.08] transition-all"
                        title="Share"
                      >
                        <Share2 size={15} />
                      </button>
                      <button
                        ...
                        className="p-2 rounded-lg text-zinc-500 hover:text-amber-400 hover:bg-amber-400/10 transition-all"
                        title="Bookmark"
                      >
                        <Bookmark size={15} />
                      </button>
