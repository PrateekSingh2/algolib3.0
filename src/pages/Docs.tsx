...
// ─── BACKGROUND ───────────────────────────────────────────────────────────────
const AmbientBg = () => (
  <div className="fixed inset-0 -z-10 bg-[#09090b] pointer-events-none overflow-hidden">
    <div className="absolute inset-0 bg-[linear-gradient(to_right,#ffffff04_1px,transparent_1px),linear-gradient(to_bottom,#ffffff04_1px,transparent_1px)] bg-[size:40px_40px]" />
    <div className="absolute -top-32 right-0 w-[60vw] h-[50vh] rounded-full bg-indigo-600/[0.07] blur-[120px]" />
    <div className="absolute bottom-0 -left-32 w-[50vw] h-[40vh] rounded-full bg-emerald-600/[0.07] blur-[100px]" />
  </div>
);

// ─── CALLOUT BLOCKS ───────────────────────────────────────────────────────────
const NoteBlock = ({ text }: { text: string }) => (
  <div className="flex gap-3 p-4 mt-5 bg-indigo-500/10 border border-indigo-500/25 rounded-xl">
    <Info className="w-4 h-4 text-indigo-400 shrink-0 mt-0.5" />
    <p className="text-sm text-indigo-200/80 leading-relaxed">{text}</p>
  </div>
);

const WarnBlock = ({ text }: { text: string }) => (
  <div className="flex gap-3 p-4 mt-5 bg-amber-500/10 border border-amber-500/25 rounded-xl">
...
const SearchTrigger = ({ onClick }: { onClick: () => void }) => (
  <button
    onClick={onClick}
    className="group w-full flex items-center justify-between gap-3 px-3 py-2.5 mb-8 bg-zinc-900/80 border border-zinc-800 rounded-lg text-sm text-zinc-500 hover:text-zinc-300 hover:border-zinc-700 transition-all"
  >
    <div className="flex items-center gap-2">
      <Search className="w-4 h-4" />
      <span>Quick search...</span>
    </div>
    <kbd className="hidden sm:inline-flex h-5 items-center rounded border border-zinc-800 bg-zinc-950 px-1.5 font-mono text-[10px] text-zinc-600 gap-1">
...
const SidebarNav = ({
  ...
  <nav className="space-y-7">
    {Object.entries(categories).map(([category, sections]) => (
      <div key={category}>
        <p className="text-[10px] font-bold uppercase tracking-widest text-zinc-600 mb-2 px-3">
          {category}
        </p>
          ...
          {sections.map(section => {
            ...
            return (
              <li key={section.id}>
                ...
                >
                  <Icon className={`w-4 h-4 shrink-0 ${isActive ? 'text-indigo-400' : 'text-zinc-600'}`} />
...
const TableOfContents = ({
  ...
  <div className="pl-6 border-l border-zinc-800/60 py-1">
    <p className="text-xs font-semibold text-zinc-300 mb-3">On this page</p>
          ...
          <li key={i}>
            <a
              ...
              className={`text-sm block leading-snug transition-colors ${
                isActive ? 'text-indigo-400 font-medium' : 'text-zinc-600 hover:text-zinc-300'
              }`}
            >
              {block.heading}
            </a>
  ...
  return (
    <div className="min-h-screen text-zinc-100 antialiased">
              ...
              {searchResults.map((r, i) => (
                ...
                >
                  <div className="flex items-center gap-2 w-full">
                    <r.section.icon className="w-4 h-4 text-indigo-400 shrink-0" />
                    <div className="flex flex-col">
                      <span className="text-sm">{r.heading}</span>
                      {r.heading !== r.section.title && (
                        <span className="text-xs text-zinc-500">{r.section.title}</span>
                      )}
                    </div>
                  </div>
                </CommandItem>
                ...
                {sections.map(section => (
                  <React.Fragment key={section.id}>
                    <CommandItem onSelect={() => navigate(section.id)}>
                      <section.icon className="w-4 h-4 mr-2 text-indigo-400" />
                      {section.title}
                    </CommandItem>
                    {section.content.map((block: any, i: number) => (
                      <CommandItem
                        ...
                        className="pl-8 text-zinc-400"
                      >
                        <Hash className="w-3 h-3 mr-2 text-zinc-600" />
        ...
        {mobileOpen && (
          ...
          >
            <motion.aside
              ...
              className="w-72 h-full bg-[#111113] border-r border-zinc-800 p-5 overflow-y-auto"
      ...
      <div className="relative max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-8 pt-24 pb-24 flex gap-0">
        ...
        {/* LEFT SIDEBAR */}
        <aside className="hidden lg:flex flex-col w-60 xl:w-64 shrink-0 sticky top-24 h-[calc(100vh-6rem)] overflow-y-auto pr-4 border-r border-zinc-800/40 mr-8">
          ...
          <div className="flex-1 min-w-0 py-2">
            <AnimatePresence mode="wait">
              ...
              >
                ...
                <p className="text-lg text-zinc-400 leading-relaxed mb-12 border-b border-zinc-800/50 pb-8">
                  {current.description}
                </p>
                  ...
                  {current.content.map((block: any, i: number) => {
                    ...
                    return (
                      <section key={i} id={hId} className="scroll-mt-28">
                        <h2 className="group flex items-center gap-2 text-xl sm:text-2xl font-bold text-zinc-100 mb-4">
                        ...
                        <div className="space-y-3">
                          {block.text.map((para: string, j: number) => (
                            <p key={j} className="text-zinc-300 leading-7 text-[15px]">{para}</p>
                          ))}
                        </div>
                ...
                {/* PAGE FOOTER NAV */}
                <div className="mt-16 pt-8 border-t border-zinc-800/50 flex justify-between text-sm">
                  {(() => {
                      ...
                      <>
                        {prev ? (
                          <button
                            ...
                            className="flex flex-col items-start gap-1 text-zinc-400 hover:text-indigo-400 transition-colors group"
                          >
                            <span className="text-xs text-zinc-600 group-hover:text-zinc-400">← Previous</span>
                        ...
                        {next ? (
                          <button
                            ...
                            className="flex flex-col items-end gap-1 text-zinc-400 hover:text-indigo-400 transition-colors group"
                          >
                            <span className="text-xs text-zinc-600 group-hover:text-zinc-400">Next →</span>
