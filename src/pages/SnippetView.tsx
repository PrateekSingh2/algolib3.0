...
const SnippetView = () => {
  ...
  const ideStyles = `
    ...
    .token.property, .token.tag, .token.boolean, .token.number, .token.constant, .token.symbol, .token.deleted { color: #f472b6; /* Pink */ }
    .token.selector, .token.attr-name, .token.string, .token.char, .token.builtin, .token.inserted { color: #34d399; /* Emerald Green */ }
    .token.operator, .token.entity, .token.url, .language-css .token.string, .style .token.string { color: #38bdf8; /* Light Blue */ }
    .token.atrule, .token.attr-value, .token.keyword { color: #c084fc; /* Purple */ }
    .token.function { color: #60a5fa; /* Blue */ font-weight: 500; }
    .token.class-name { color: #fbbf24; /* Amber/Yellow */ font-weight: 500; }
               ...
               >
                  {algorithm.tags?.map((tag) => (
                     <Link 
                        key={tag} 
                        to={`/?search=${tag}`}
                        className="px-3 py-1.5 bg-[#121214] border border-zinc-800 hover:border-zinc-600 hover:bg-zinc-800 transition-all rounded-lg text-xs font-medium text-zinc-400 flex items-center gap-1"
                     >
