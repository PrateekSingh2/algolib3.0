...
const Section = ({ num, title, icon: Icon, children }: any) => (
  <div className="mb-16 last:mb-0 relative">
    <div className="absolute -left-8 top-1 hidden md:block text-white/10 font-mono text-xl font-bold">{num}</div>
    ...
    <div className="text-zinc-400 font-light leading-relaxed space-y-6">
      {children}
    </div>
  </div>
);
...
const Terms = () => {
  return (
    <LegalLayout title="Terms of Service" lastUpdated="March 2026">
      ...
      <div className="w-full h-[1px] bg-gradient-to-r from-transparent via-white/[0.05] to-transparent my-12" />
      ...
      <Section num="03" title="User Accounts & Authentication" icon={Lock}>
        ...
        <div className="bg-white/[0.02] border border-white/[0.05] rounded-2xl p-6 mt-4 space-y-4">
          {[
            ...
            <div key={i} className="flex items-start gap-3">
              <ChevronRight className="w-4 h-4 text-[#00d2ff] shrink-0 mt-1" />
      ...
      <Section num="04" title="Acceptable Use Policy" icon={AlertTriangle}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-6">
          <div className="p-5 rounded-xl bg-red-500/[0.02] border border-red-500/[0.05]">
            <h4 className="text-white text-sm font-medium mb-2">Security Bypassing</h4>
            <p className="text-xs">Attempting to bypass our authentication guards or Row Level Security (RLS).</p>
          </div>
          <div className="p-5 rounded-xl bg-red-500/[0.02] border border-red-500/[0.05]">
            <h4 className="text-white text-sm font-medium mb-2">Data Scraping</h4>
            <p className="text-xs">Mining or extracting data from the algorithm library without explicit API permission.</p>
          </div>
          <div className="p-5 rounded-xl bg-red-500/[0.02] border border-red-500/[0.05] md:col-span-2">
            <h4 className="text-white text-sm font-medium mb-2">Malicious Content</h4>
