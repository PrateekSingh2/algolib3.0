...
import { 
  ...
  ChevronLeft, Send, CheckCircle2, LayoutGrid, AlertOctagon, Maximize, Trophy, Target, Loader2, Lock, SearchX, Calendar, Info, Check, Hash
} from 'lucide-react';
...
export default function QuizArena() {
  ...
  // Real-time Leaderboard Polling Logic
  useEffect(() => {
  ...
  // ---------------------------------------------------------
  // REUSABLE LEADERBOARD UI WIDGET
  // ---------------------------------------------------------
  const renderLeaderboardCard = () => (
    <motion.div initial={{ scale: 0.95, opacity: 0, x: 20 }} animate={{ scale: 1, opacity: 1, x: 0 }} transition={{ delay: 0.2 }} className="max-w-md w-full bg-[#0a0a0a]/90 backdrop-blur-3xl border border-white/[0.08] p-8 rounded-[2.5rem] shadow-2xl flex flex-col h-[500px]">
        <div className="flex items-center justify-between mb-6">
            <h2 className="text-xl font-bold flex items-center gap-2"><Trophy size={20} className="text-amber-400" /> Live Leaderboard</h2>
            <div className="flex items-center gap-1.5 text-[11px] font-bold tracking-widest uppercase text-emerald-400 bg-emerald-500/10 px-3 py-1.5 rounded-full border border-emerald-500/20">
                <span className="w-1.5 h-1.5 rounded-full bg-emerald-400 animate-pulse"></span> Auto-Sync
            </div>
        </div>
        <div className="space-y-3 flex-1 overflow-y-auto pr-2" style={{ scrollbarWidth: 'thin', scrollbarColor: '#3f3f46 transparent' }}>
             ...
             <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-md w-full bg-[#0a0a0a]/90 backdrop-blur-3xl border border-white/[0.08] p-10 rounded-[2.5rem] shadow-2xl text-center flex flex-col justify-center h-[500px]">
                ...
                {eligibility === 'denied' && (
                  <div className="bg-black/50 border border-white/[0.05] p-6 rounded-[2rem] flex flex-col items-center text-center mx-auto mb-8 shadow-inner w-full">
                     <p className="text-xs text-zinc-500 font-bold uppercase tracking-widest mb-3">Logged Score</p>
  ...
  if (!hasStarted) {
    return (
      <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center px-6 relative overflow-hidden">
        ...
        <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="max-w-2xl w-full bg-[#0a0a0a]/80 backdrop-blur-3xl border border-white/[0.08] p-10 md:p-12 rounded-[2.5rem] shadow-2xl relative z-10 mt-16">
           ...
           <div className="flex items-start gap-5 text-sm bg-rose-500/5 p-6 rounded-[1.5rem] border border-rose-500/20 mb-10 mt-4 shadow-inner">
              ...
              <div className="space-y-2 text-zinc-300">
                  ...
                  <p className="leading-relaxed text-zinc-400">This is a highly secure environment. Losing focus on this window, accessing external applications, or exiting fullscreen will register as a critical violation.</p>
                  <div className="inline-flex items-center gap-2 px-3 py-1.5 bg-rose-500/10 text-rose-400 font-bold rounded-lg mt-2 border border-rose-500/20"><AlertTriangle size={14}/> Max Violations Allowed: {quizData.maxWarnings}</div>
              </div>
           </div>
      ...
      <div className="min-h-screen bg-[#050505] text-white flex flex-col items-center justify-center px-6 relative overflow-hidden">
        ...
        <div className="flex flex-col lg:flex-row items-center lg:items-stretch justify-center gap-6 w-full max-w-5xl relative z-10 mt-20 lg:mt-0">
           <motion.div initial={{ scale: 0.95, opacity: 0 }} animate={{ scale: 1, opacity: 1 }} className="max-w-xl w-full bg-[#0a0a0a]/80 backdrop-blur-3xl border border-white/[0.08] p-10 md:p-10 rounded-[2.5rem] shadow-2xl text-center flex flex-col h-[500px]">
              ...
              <p className="text-zinc-400 text-sm mb-10 max-w-sm mx-auto leading-relaxed">Your behaviour, activity and responses have been successfully logged. You may now securely exit this session.</p>
              
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 text-left mb-auto">
                <div className="bg-white/[0.02] border border-white/[0.05] p-5 rounded-2xl flex flex-col items-center text-center shadow-inner"><Trophy size={20} className="text-amber-400 mb-2"/><p className="text-[10px] text-zinc-500 font-bold uppercase tracking-widest mb-1">Final Score</p><p className="text-2xl font-mono text-white tracking-tighter font-bold">{finalScore} <span className="text-xs text-zinc-600 font-normal tracking-normal">/ {quizData.questions.length}</span></p></div>
  ...
  return (
    <div className="min-h-screen bg-[#050505] text-zinc-100 font-sans flex flex-col overflow-x-hidden relative">
      ...
      <AnimatePresence>
        {showWarningModal && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/90 backdrop-blur-xl p-6">
             <motion.div initial={{ scale: 0.9, y: 20 }} animate={{ scale: 1, y: 0 }} className="max-w-md w-full bg-[#0a0a0a] border border-rose-500/30 p-10 rounded-[2.5rem] shadow-[0_0_100px_rgba(244,63,94,0.15)] text-center relative overflow-hidden">
                <div className="absolute top-0 left-0 w-full h-1 bg-rose-500 animate-pulse"></div>
                <AlertOctagon size={64} className="text-rose-500 mx-auto mb-6 drop-shadow-[0_0_15px_rgba(244,63,94,0.5)]" />
                <h2 className="text-3xl font-extrabold text-white mb-3">Protocol Breach</h2>
                <p className="text-zinc-400 text-sm mb-6 leading-relaxed">System logged a violation: <span className="text-white font-semibold">{warningReason}</span>. Further breaches will initiate forced termination.</p>
                <div className="bg-rose-500/10 border border-rose-500/20 text-rose-400 py-4 rounded-xl mb-8 font-mono text-sm font-bold tracking-widest uppercase">Strike: {warningsRef.current} of {quizData.maxWarnings}</div>
                <button onClick={acknowledgeWarning} className="w-full py-4 bg-rose-600 text-white font-bold rounded-2xl transition-all hover:bg-rose-500 shadow-[0_0_30px_rgba(225,29,72,0.4)] text-lg active:scale-[0.98]">Acknowledge & Return</button>
             </motion.div>
          </motion.div>
        )}
        ...
        {showSubmitConfirm && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[200] flex items-center justify-center bg-black/80 backdrop-blur-md p-6">
             <motion.div initial={{ scale: 0.95, y: 20 }} animate={{ scale: 1, y: 0 }} className="max-w-sm w-full bg-[#111] border border-white/[0.08] p-10 rounded-[2.5rem] shadow-2xl text-center">
                <Send size={48} className="text-indigo-400 mx-auto mb-6" />
                <h2 className="text-2xl font-bold text-white mb-2">Finalize Sequence</h2>
                <p className="text-zinc-400 text-sm mb-8 leading-relaxed">Are you certain you wish to submit? Telemetry will be locked and answers cannot be altered.</p>
                <div className="flex flex-col gap-3">
                   <button 
                      onClick={() => { setIsSubmittingToDB(true); finalizeSubmission(); }}
                      disabled={isSubmittingToDB}
                      className="w-full py-4 bg-indigo-500 text-white font-bold text-lg rounded-2xl hover:bg-indigo-400 transition-all shadow-[0_0_20px_rgba(99,102,241,0.3)] disabled:opacity-50 flex items-center justify-center gap-2"
                   >
      ...
      <header className="fixed top-0 left-0 w-full z-40 bg-[#050505]/80 backdrop-blur-3xl border-b border-white/[0.05]">
      ...
      <main className="flex-1 flex justify-center px-4 md:px-6 pt-24 pb-40 relative z-10 w-full max-w-5xl mx-auto">
         <div className="w-full mt-6 md:mt-10 flex flex-col items-center">
