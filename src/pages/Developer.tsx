...
const ApexBackground = () => (
  <div className="fixed inset-0 z-0 pointer-events-none bg-[#020202] flex items-center justify-center overflow-hidden">
    ...
    {/* Removed flickering, increased base opacity slightly for a richer backdrop */}
    <div className="absolute top-[-10%] right-[5%] w-[60vw] h-[60vh] bg-[#00d2ff] rounded-full blur-[200px] mix-blend-screen opacity-[0.12]" />
    <div className="absolute bottom-[-20%] left-[-10%] w-[70vw] h-[70vh] bg-[#7000ff] rounded-full blur-[200px] mix-blend-screen opacity-[0.12]" />
  </div>
);
...
const TeamMemberCard = ({ member, index }: { member: TeamMember; index: number }) => {
  ...
  return (
    <motion.div
      ...
      className="group relative flex flex-col h-full w-full max-w-[360px] rounded-[2.5rem] bg-white/[0.02] backdrop-blur-3xl border border-white/[0.08] border-t-white/[0.2] border-l-white/[0.15] overflow-hidden hover:border-white/[0.25] transition-all duration-500 hover:-translate-y-3 shadow-[0_8px_32px_0_rgba(0,0,0,0.5)] hover:shadow-[0_30px_60px_-15px_rgba(0,210,255,0.25)]"
    >
      {/* Glossy Diagonal Overlay */}
      <div className="absolute inset-0 bg-gradient-to-tr from-transparent via-white/[0.05] to-white/[0.12] opacity-40 group-hover:opacity-100 transition-opacity duration-700 pointer-events-none z-10" />
      ...
      <div className="relative z-30 flex-1 flex flex-col p-8 md:p-10">
        ...
        <div className="flex flex-col items-center text-center mb-8">
          <div className="relative mb-7 group-hover:scale-110 transition-transform duration-500 ease-out">
            ...
            <div className="absolute -bottom-2 -right-2 bg-white/[0.05] p-2.5 rounded-2xl border border-white/[0.2] border-t-white/[0.4] border-l-white/[0.3] z-40 shadow-[0_8px_16px_rgba(0,0,0,0.5)] backdrop-blur-xl group-hover:border-sky-400/50 group-hover:bg-sky-500/10 transition-all duration-300 group-hover:scale-110">
              <member.icon size={16} className="text-zinc-300 group-hover:text-sky-300 transition-colors" />
            </div>
          </div>
          
          <h3 className="text-2xl font-bold text-white tracking-tight leading-none mb-3 drop-shadow-md">{member.name}</h3>
          
          {/* Glossy Role Pill */}
          <span className="text-[11px] font-bold tracking-widest text-sky-100 bg-gradient-to-r from-sky-500/20 to-indigo-500/20 border border-white/20 border-t-white/40 px-4 py-1.5 rounded-full uppercase shadow-[inset_0px_1px_4px_rgba(255,255,255,0.4)] backdrop-blur-md group-hover:border-sky-400/60 transition-colors">
            {member.role}
          </span>
        </div>

        {/* Bio Text */}
        <p className="text-[15px] text-zinc-300 leading-relaxed font-light text-center mb-6 flex-1 group-hover:text-white transition-colors duration-500 drop-shadow-sm">
          {member.bio}
        </p>

        {/* Glossy Divider */}
        <div className="h-px w-full bg-gradient-to-r from-transparent via-white/[0.2] to-transparent my-6 shadow-[0_1px_2px_rgba(0,0,0,0.5)]" />

        {/* Social Links */}
        <div className="flex items-center justify-center gap-4 mt-auto relative z-40">
          <a href={member.github} target="_blank" rel="noreferrer" className="flex items-center justify-center w-11 h-11 rounded-full bg-white/[0.05] border border-white/[0.1] border-t-white/[0.3] text-zinc-300 hover:text-white hover:bg-white/[0.15] transition-all shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_0_20px_rgba(255,255,255,0.2)] hover:scale-110 active:scale-95 backdrop-blur-md">
            <Github size={18} />
          </a>
          <a href={member.linkedin} target="_blank" rel="noreferrer" className="flex items-center justify-center w-11 h-11 rounded-full bg-white/[0.05] border border-white/[0.1] border-t-white/[0.3] text-zinc-300 hover:text-[#0a66c2] hover:bg-white/[0.15] transition-all shadow-[0_4px_12px_rgba(0,0,0,0.3)] hover:shadow-[0_0_20px_rgba(10,102,194,0.4)] hover:scale-110 active:scale-95 backdrop-blur-md">
            <Linkedin size={18} />
          </a>
        </div>
...
return (
    <div className="min-h-screen bg-[#030303] text-white font-sans selection:bg-sky-500/30 flex flex-col relative overflow-hidden">
      ...
      <div className="fixed top-0 left-0 w-full z-[100] bg-black/20 backdrop-blur-2xl border-b border-white/[0.1] shadow-[0_4px_30px_rgba(0,0,0,0.5)]">
        <GlobalRibbon />
        <Navbar />
      </div>
      ...
      <main className="flex-1 relative z-10 w-full max-w-7xl mx-auto px-6 pt-40 pb-32">
        ...
        <div className="mb-14">
          <Link 
            to="/" 
            className="inline-flex items-center gap-2 px-5 py-2.5 rounded-full bg-white/[0.03] border border-white/[0.1] border-t-white/[0.25] text-zinc-300 hover:text-white hover:bg-white/[0.08] transition-all duration-300 text-sm font-medium group shadow-[0_4px_16px_rgba(0,0,0,0.4)] backdrop-blur-xl hover:shadow-[0_8px_24px_rgba(255,255,255,0.05)]"
          >
            <ArrowLeft size={16} className="text-zinc-400 group-hover:text-white group-hover:-translate-x-1 transition-all duration-300" />
        ...
        >
          <div className="inline-flex items-center justify-center gap-2 px-4 py-2 rounded-full border border-sky-400/30 border-t-sky-300/50 bg-gradient-to-b from-sky-500/10 to-transparent mb-8 backdrop-blur-xl shadow-[0_8px_32px_rgba(14,165,233,0.15)]">
             ...
             <span className="text-xs font-bold tracking-widest text-sky-300 uppercase drop-shadow-md">
               Architecture Crew
             </span>
          </div>
          ...
          <h1 className="text-6xl md:text-8xl font-extrabold tracking-tighter mb-8 leading-[1.05]">
            ...
            <span className="text-transparent bg-clip-text bg-gradient-to-br from-white via-sky-200 to-indigo-400 drop-shadow-[0_0_50px_rgba(56,189,248,0.3)]">
              Visionaries.
            </span>
          </h1>

          <p className="text-zinc-300 text-lg md:text-2xl font-light leading-relaxed max-w-2xl mx-auto drop-shadow-sm">
      ...
      <div className="relative z-10 border-t border-white/[0.08] bg-black/40 backdrop-blur-xl">
        <AppFooter />
      </div>
    </div>
  );
